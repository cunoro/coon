...
contract CunoroBondDepository is Ownable {
    ...
    function deposit(
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) NORO
         */
        ...
        if (fee != 0) {
            ...
            IERC20(NORO).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(NORO).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(NORO).approve(stakingHelper, _amount);
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice()
                .mul(
                    ICunoroBondingCalculator(bondCalculator).markdown(principle)
                )
                .div(100);
        } else {
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return
                debtRatio()
                    .mul(
                        ICunoroBondingCalculator(bondCalculator).markdown(
                            principle
                        )
                    )
                    .div(1e9);

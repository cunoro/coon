...
contract sCunoro is IsNORO, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgNORO public gNORO; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked NORO", "sNORO", 9) ERC20Permit("Staked NORO") {
    ...
    function setgNORO(address _gNORO) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gNORO) == address(0), "gNORO:  gNORO already set");
        require(_gNORO != address(0), "gNORO:  gNORO is not a valid contract");
        gNORO = IgNORO(_gNORO);
    }
    ...
    /**
        @notice increases rNORO supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sNORO of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sNORO
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sNORO: insufficient balance");
    }
    ...
    // toG converts an sNORO balance to gNORO terms. gNORO is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gNORO.balanceTo(amount);
    }

    // fromG converts a gNORO balance to sNORO terms. sNORO is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gNORO.balanceFrom(amount);
    }

    // Staking contract holds excess sNORO
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gNORO.balanceFrom(IERC20(address(gNORO)).totalSupply())).add(

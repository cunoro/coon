...
import {CunoroAccessControlledV2, ICunoroAuthority} from "../types/CunoroAccessControlledV2.sol";

/**
    @title INOROIndexWrapper
    @notice This interface is used to wrap cross-chain oracles to feed an index without needing IsNORO, 
    while also being able to use sNORO on mainnet.
 */
interface INOROIndexWrapper {
...
/**
    @title YieldSplitter
    @notice Abstract contract that allows users to create deposits for their gNORO and have
            ...
            emergency controls, sending and recieving gNORO is up to the implementation of
            this abstract contract to handle.
 */
abstract contract YieldSplitter is CunoroAccessControlledV2, IYieldSplitter {
    using SafeERC20 for IERC20;

    INOROIndexWrapper public immutable indexWrapper;

    struct DepositInfo {
        uint256 id;
        address depositor;
        uint256 principalAmount; // Total amount of sNoro deposited as principal, 9 decimals.
        uint256 agnosticAmount; // Total amount deposited priced in gNoro. 18 decimals.
    }
    ...
    /**
        @notice Constructor
        @param indexWrapper_ Address of contract that will return the sNORO to gNORO index. 
                             On mainnet this will be sNORO but on other chains can be an oracle wrapper.
    */
    constructor(address indexWrapper_, address authority_) CunoroAccessControlledV2(ICunoroAuthority(authority_)) {
        indexWrapper = INOROIndexWrapper(indexWrapper_);
    }
    ...
    */
    ...
    /**
        @notice Convert flat sNORO value to agnostic gNORO value at current index
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e18 is because sNORO has 9 decimals, gNORO has 18 and index has 9.
     */
    ...
    /**
        @notice Convert agnostic gNORO value at current index to flat sNORO value
        @dev Agnostic value earns rebases. sNORO amount is gNOROamount * rebase_index.
             1e18 is because sNORO has 9 decimals, gNORO has 18 and index has 9.
     */

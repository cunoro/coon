...
contract CunoroBondingCalculator is IBondingCalculator {
    ...
    function getKValue(address _pair) public view returns (uint256 k_) {
        uint256 token0 = IERC20Metadata(IJoePair(_pair).token0()).decimals();
        uint256 token1 = IERC20Metadata(IJoePair(_pair).token1()).decimals();
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IJoePair(_pair).token0() == address(NORO)) {
            reserve = reserve1;
        } else {
            require(IJoePair(_pair).token1() == address(NORO), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(NORO)).decimals())).div(getTotalValue(_pair));
    }
}

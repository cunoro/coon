...
describe("sNoro", () => {
  ...
  beforeEach(async () => {
    ...
    const authority = await (new CunoroAuthority__factory(initializer)).deploy(initializer.address, initializer.address, initializer.address, initializer.address);
  ...
  describe("initialization", () => {
    ...
    describe("initialize", () => {
      ...
      it("emits Transfer event", async () => {
        await expect(sNoro.connect(initializer).initialize(stakingFake.address, treasuryFake.address)).
          to.emit(sNoro, "Transfer").withArgs(ZERO_ADDRESS, stakingFake.address, TOTAL_GONS);
      });

      it("emits LogStakingContractUpdated event", async () => {
        await expect(sNoro.connect(initializer).initialize(stakingFake.address, treasuryFake.address)).
          to.emit(sNoro, "LogStakingContractUpdated").withArgs(stakingFake.address);
      });

      it("unsets the initializer, so it cannot be called again", async () => {
        await sNoro.connect(initializer).initialize(stakingFake.address, treasuryFake.address);
        await expect(sNoro.connect(initializer).initialize(stakingFake.address, treasuryFake.address)).to.be.reverted;
      });
    });
  });
    ...
    describe("approve", () => {
      ...
      it("emits an Approval event", async () => {
        await expect(await sNoro.connect(alice).approve(bob.address, 10)).
          to.emit(sNoro, "Approval").withArgs(alice.address, bob.address, 10);
      });
    });
    ...
    describe("increaseAllowance", () => {
      ...
      it("emits an Approval event", async () => {
        ...
        await expect(await sNoro.connect(alice).increaseAllowance(bob.address, 4)).
          to.emit(sNoro, "Approval").withArgs(alice.address, bob.address, 14);
      });
    });
    ...
    describe("decreaseAllowance", () => {
      ...
      it("emits an Approval event", async () => {
        ...
        await expect(await sNoro.connect(alice).decreaseAllowance(bob.address, 4)).
          to.emit(sNoro, "Approval").withArgs(alice.address, bob.address, 6);
      });
    });
